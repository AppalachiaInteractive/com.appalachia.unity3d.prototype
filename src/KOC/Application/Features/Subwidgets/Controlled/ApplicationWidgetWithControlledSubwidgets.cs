...
namespace Appalachia.Prototype.KOC.Application.Features.Subwidgets.Controlled
{
    ...
    public abstract class ApplicationWidgetWithControlledSubwidgets<
        TSubwidget, TWidget, TWidgetMetadata, TFeature, TFeatureMetadata, TFunctionalitySet, TIService,
        TIWidget, TManager> : ApplicationWidget<TWidget, TWidgetMetadata, TFeature, TFeatureMetadata,
        ...
        where TWidget : ApplicationWidgetWithControlledSubwidgets<TSubwidget, TWidget, TWidgetMetadata,
            TFeature, TFeatureMetadata, TFunctionalitySet, TIService, TIWidget, TManager>, TIWidget
        where TWidgetMetadata : ApplicationWidgetWithControlledSubwidgetsMetadata<TSubwidget, TWidget,
    ...
    {
        static ApplicationWidgetWithControlledSubwidgets()
        {
            void RespondToSubwidgetBeingEnabled(AppaEvent<TSubwidget>.Args subwidget)
            {
                ...
                RegisterInstanceCallbacks
                   .For<ApplicationWidgetWithControlledSubwidgets<TSubwidget, TWidget, TWidgetMetadata,
        ...
        public void DiscardNullSubwidgets()
        {
            using (_PRF_DiscardNullSubwidgets.Auto())
            {
                for (var index = _subwidgets.Count - 1; index >= 0; index--)
                {
                    var subwidget = _subwidgets[index];

                    if (subwidget == null)
                    {
                        _subwidgets.RemoveAt(index);
                    }
                }
            }
        }
        ...
        private static readonly ProfilerMarker _PRF_DiscardNullSubwidgets =
            new ProfilerMarker(_PRF_PFX + nameof(DiscardNullSubwidgets));
