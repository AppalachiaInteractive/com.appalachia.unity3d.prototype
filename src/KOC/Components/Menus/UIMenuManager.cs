...
using Appalachia.Prototype.KOC.Components.Menus.Metadata.Wrappers;
...
namespace Appalachia.Prototype.KOC.Components.Menus
{
    [ExecuteAlways]
            ...
            using (_PRF_UpdateMenus.Auto())
            {
                if (_backgrounds != null)
                {
                    for (var index = 0; index < _backgrounds.Count; index++)
                    {
                        ...
                        metadata.Apply(gameObject, backgroundName, components);
                    }
                }
                ...
                if (_buttons != null)
                {
                    for (var index = 0; index < _buttons.Count; index++)
                    {
                        ...
                        var buttonName = GetButtonBaseName(metadata, index);
                        metadata.Apply(gameObject, buttonName, components);
                    }
                }
            }
        }
