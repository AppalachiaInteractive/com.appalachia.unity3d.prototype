...
namespace Appalachia.Prototype.KOC.Areas.DeveloperInterface.V01.Features.Profiling.Services.Audio
{
    [CallStaticConstructorInEditor]
    public sealed class AudioProfilerService : DeveloperInterfaceManager_V01.Service<AudioProfilerService,
        AudioProfilerServiceMetadata, PerformanceProfilingFeature, PerformanceProfilingFeatureMetadata>
    {
            ...
            using (_PRF_Update.Auto())
            {
                ...
                if (_audioListener != null)
                {
                    ...
                    for (var i = 0; i < Spectrum.Length; i++)
                    {
                        ...
                        {
                            var decayRate = CoreClock.Instance.DeltaTime * 2;
                            var highestValue = SpectrumHighestValues[i];
                            
                            SpectrumHighestValues[i] = Mathf.Clamp(
                                highestValue -
                                (highestValue * decayRate),
                                0,
                                1
                            );
                        }
        ...
        public void ValidateArraySizes()
        {
            ...
            if ((SpectrumHighestValues == null) || (Metadata.spectrumSize != SpectrumHighestValues.Length))
            {
